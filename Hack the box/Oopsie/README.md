...
Burp Suite was used as a passive proxy to map the application during normal browsing. This revealed additional paths not visible from the homepage, including:

/cdn-cgi/login
![Burp Sitemap](images/burp-sitemap.png)
B) Login Discovery

Browsing to /cdn-cgi/login exposed the application login page.

Standard login attempts were unsuccessful, but the application allowed access through a guest login option.
...
http://10.129.143.127/cdn-cgi/login/admin.php?content=accounts&id=2
...
Administrative access exposed a file upload feature that accepted user-controlled files.

This functionality was used to upload a PHP reverse shell to the target.
...
A) Privilege Context Enumeration
...
This revealed a binary located at:

/usr/bin/bugtracker
C) Bugtracker Analysis
